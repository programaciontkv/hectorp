              ...
              <tr>
                ...
                <td >
                  <div class="form-group <?php if (form_error('nombre') != '') {
                    ...
                    <input type="text" class="form-control" name="nombre" id="nombre" value="<?php if (validation_errors() != '') {
              ...
              <tr>
                ...
                <td >
                  <div class="form-group <?php if (form_error('direccion_cliente') != '') {
                    ...
                    <input type="text" class="form-control" name="direccion_cliente" id="direccion_cliente" value="<?php if (validation_errors() != '') {
            ...
            <td class="col-sm-16" colspan="2">
              ...
              <table class="table table-bordered table-striped" width="100%"  id="tbl_detalle">
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td width="70px">
                              <input type ="text" size="7" style="text-align:right" id="pro_subsidio" name="pro_subsidio" onchange="calculo_encabezado(this)" value="" lang="1" class="form-control decimal"/>
                            </td>
                            <td width="70px">
                        ...
                        foreach ($cns_det as $rst_det) {
                          ...
                          ?>
                                <tr>
                                    ...
                                    <td><input type ="text" size="7" style="text-align:right" class="form-control decimal" id="<?php echo 'pro_subsidio' . $n ?>" name="<?php echo 'pro_subsidio' . $n ?>" value="<?php echo str_replace(',', '', number_format($rst_det->pro_subsidio, $dec)) ?>" lang="<?PHP echo $n ?>" onchange="calculo(this)" /></td>
                                    <td>
                ...
                <tfoot>
                    ...
                    <tr>
                        <td colspan="2" align="right">Valor sin subsidio:</td>
                        <td><input style="text-align:right;font-size:15px;color:red  " type="text" class="form-control" id="sin_subsidio" name="sin_subsidio" value="<?php echo str_replace(',', '', number_format($factura->fac_sin_subsidio, $dec)) ?>" readonly />
                            
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2" align="right">Ahorro por subsidio:</td>
                        <td><input style="text-align:right;font-size:15px;color:red  " type="text" class="form-control" id="ahorro_subsidio" name="ahorro_subsidio" value="<?php echo str_replace(',', '', number_format($factura->fac_total_valor-$factura->fac_sin_subsidio, $dec)) ?>" readonly />
                          <input style="text-align:right;font-size:15px;color:red  " type="hidden" class="form-control" id="total_subsidio" name="total_subsidio" value="<?php echo str_replace(',', '', number_format($factura->fac_total_subsidio, $dec)) ?>" readonly />
...
function clona_detalle(table,opc) {
    ...
    if (d == 0) {
        ...
        var fila = "<tr>"+
                            ...
                            "</td>"+
                            "<td>"+
                              "<input type ='text' size='7' style='text-align:right' id='pro_subsidio"+i+"' name='pro_subsidio"+i+"' onchange='calculo()' value='"+pro_subsidio.value+"' lang='"+i+"' class='form-control decimal' onkeyup='validar_decimal(this)' />"+
                            "</td>"+
                            "<td style='width:50px'>"+
      ...
      beforeSend: function () {
        ...
        success: function (dt) {
          if (dt!='') {
            ...
            if (dt.pro_subsidio == '' || dt.pro_subsidio == null ) {
              $('#pro_subsidio').val('0');
            }else{
              $('#pro_subsidio').val(dt.pro_subsidio);
            }
...
function calculo(obj) {
    ...
    while (n < i) {
        ...
        if ($('#item' + n).val() == null) {
            ...
            subsd=$('#pro_subsidio' + n).val().replace(',', '');
            if(cnt==''){
              cnt=0;
            }
            ...
            ///valor con subsidio
            vsb=round(cnt,dcc) * (round(pr,dec)+round(subsd,dec));
            vtsb = (vsb * 1) - (vsb * round(d,dec) / 100);
            dscsb= (round(vtsb,dec) * round(d,dec)) / 100; 
            if(dscsb.toFixed(2)=='NaN'){
              dscsb=0;
            }  
            tsubsidio=tsubsidio+round(cnt,dcc)*round(subsd,dec);
        ...
        //valores con subsidio
        tdscsb = (round(tdscsb,dec) * 1) + (round(dscsb,dec) * 1);

        if (ob == '14') {
            ts12 = (round(ts12,dec) * 1 + round(vtsb,dec) * 1);
            tivas = (round(ts12,dec) * 14 / 100);
        }


        if (ob == '12') {
            ts12 = (round(ts12,dec) * 1 + round(vtsb,dec) * 1);
            tivas = (round(ts12,dec) * 12 / 100);
        }

        if (ob == '0') {
            ts0 = (round(ts0,dec) * 1 + round(vtsb,dec) * 1);
        }
        if (ob == 'EX') {
            texs = (round(texs,dec) * 1 + round(vtsb,dec) * 1);
        }
        if (ob == 'NO') {
            tnos = (round(tnos,dec) * 1 + round(vtsb,dec) * 1);
        }
    ...
    //valores con subsidio
    subs = round(ts12,dec) + round(ts0,dec) + round(texs,dec) + round(tnos,dec);
    sinsubsidio = (round(subs,dec) * 1 + round(tivas,dec) * 1 + round(prop,dec) * 1);
    ahsubsidio=round(sinsubsidio,dec)-round(gtot,dec);
    ...
    $('#sin_subsidio').val(sinsubsidio.toFixed(dec));
    $('#ahorro_subsidio').val(ahsubsidio.toFixed(dec));
    $('#total_subsidio').val(tsubsidio.toFixed(dec));
                ...
                while (n < i) {
                    ...
                    if ($('#pro_descripcion' + n).html() != null) {
                        if ($('#pro_descripcion' + n).html().length == 0) {
                            ...
                            $('#guardar').attr('disabled',false);
                            return false;
                        } else if ($('#pro_subsidio' + n).val().length == 0) {
                            $('#pro_subsidio' + n).css({borderColor: "red"});
                            $('#pro_subsidio' + n).focus();
                            $("#guardar").attr('disabled',false);
                            return false;

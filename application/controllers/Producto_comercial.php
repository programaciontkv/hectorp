...
class Producto_comercial extends CI_Controller {
	...
	public function guardar($opc_id){
		...
		$pro_subsidio = $this->input->post('mp_t');
		...
		$this->form_validation->set_rules('mp_t','subsidio','required');
		if($this->form_validation->run()){
			$data=array(
					                        ...
					                        'mp_aa'=>$pro_imagen,
					                        'mp_i'=>$pro_estado,
											'mp_t'=>$pro_subsidio///pro_subsidio
	...
	public function show_frame($id,$opc_id){
     ...
     public function show_pdf($id,$opc_id){
    		...
    		$imagen=$this->set_barcode(strtoupper($pro->mp_c)); 
			$data=array(
						'producto'=>$this->producto_comercial_model->lista_un_producto($id)
						);

...
class Factura extends CI_Controller {
	...
	public function index($opc_id){
		...
		if (($rst_opc->opc_id)=='85') {
			...
			$data=array(
						...
						'url'=>base_url().strtolower($rst_opc->opc_direccion).$rst_opc->opc_id,
			);
	...
	public function guardar($opc_id){
		...
		$subsidio= $this->input->post('sin_subsidio');
		$total_subsidio= $this->input->post('total_subsidio');
		...
		if($this->form_validation->run()){
		    ...
		    $data=array(	
							...
							'fac_sin_subsidio'=>$subsidio,
							'fac_total_subsidio'=>$total_subsidio,
							'fac_estado'=>'4'
		    );
		    ...
		    if(!empty($fac_id)){
		    	...
		    	while($n<$count_det){
		    		...
		    		if($this->input->post("pro_aux$n")!=''){
		    			...
		    			$dfc_subsidio = $this->input->post("pro_subsidio$n");
		    			$dfc_sin_subsidio = $dfc_subsidio+$dfc_precio_unit;
		    			$dt_det=array(
		    							'fac_id'=>$fac_id,
	                                    'pro_id'=>$pro_id,
	                                    ...
	                                    'dfc_p_ice'=>0, 
	                                    'dfc_cod_ice'=>0,
	                                    'dfc_subsidio'=>$dfc_subsidio,
	                                    'dfc_sin_subsidio'=>$dfc_sin_subsidio,
		    						);
...
public function buscar_cliente_3($txt){
	...
	$cns=$this->cliente_model->lista_un_cliente3($txt);
	$n=1;
    ...
    public function show_pdf($id,$opc_id,$correo,$etiqueta){
			...
			$dt_det=(object) array(
						...
						'ice_cod'=>$rst_dt->dfc_cod_ice,
						'precio_tot'=>$rst_dt->dfc_precio_total,
						'pro_subsidio'=>$rst_dt->dfc_subsidio,
						'pro_sin_subsidio'=>$rst_dt->dfc_sin_subsidio,
						);	
    ...
    //ENCABEZADO
        ...
        if($factura->fac_total_subsidio>0){
        	$xml.="<totalSubsidio>". round($factura->fac_total_subsidio, $round) ."</totalSubsidio>" . chr(13);
    	}
        ...
        foreach ($detalle as $det) {
            ...
            if($det->dfc_subsidio>0){
            	$xml.="<precioSinSubsidio>" . round($det->dfc_sin_subsidio, $round) . "</precioSinSubsidio>" . chr(13);
        	}
   ...
   public function show_pdf_2($id,$opc_id,$correo,$etiqueta,$tip){
    ...
    public function mail_no_enviados($opc){
        
        $factura=$this->factura_model->facturas_no_enviadas();
        if(!empty($factura->fac_id)){
        	$this->consulta_sri($factura->fac_id,$opc,0);
        }
